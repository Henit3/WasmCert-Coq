...
Proof.
  ...
  - rewrite - catA. f_equal.
    ...
    + by eapply IHHType.
Qed.
Lemma AI_const_vec_typing: forall s C v t1s t2s,
...
Proof.
  ...
  - rewrite - catA. f_equal.
    ...
    + by eapply IHHType.
Qed.
Lemma AI_ref_null_typing: forall s C v t1s t2s,
...
Lemma AI_ref_func_typing: forall s C v t1s t2s,
...
Ltac invert_be_typing_step:=
  ...
  | H: be_typing _ [:: BI_ref_func _] _ |- _ =>
    ...
    let H2 := fresh "H2" in
    let H3 := fresh "H3" in
    apply Ref_func_typing in H; destruct H as [tf [H1 [H2 H3]]]; subst
...
Proof.
  ...
  remove_bools_options => //=.
  move/eqP in H0. move/eqP in H1. by subst.
Qed.
...
Proof.
  ...
  unfold inst_typing, typing.inst_typing in * => //=;
  remove_bools_options; simpl in * => //=.
  unfold memi_typing, lookup_N in H2.
...
Proof.
  ...
  unfold inst_typing, typing.inst_typing in * => //=;
  remove_bools_options; simpl in * => //=.
  unfold tabi_typing, lookup_N in H3.
...
Proof.
  ...
  unfold inst_typing, typing.inst_typing in * => //=;
  remove_bools_options; simpl in * => //=.
  unfold elemi_typing, lookup_N in H1.
...
Proof.
  ...
  unfold inst_typing, typing.inst_typing in * => //=;
  remove_bools_options; simpl in * => //=.
  unfold datai_typing, lookup_N in H0.
...
Proof.
  ...
  unfold tabi_typing in H3.
  eapply all2_projection in H3; eauto.
  remove_bools_options; eauto.
  unfold lookup_N in Hoption.
  rewrite Hoption in HSLU. inversion HSLU. subst.
  unfold tab_typing in H7. remove_bools_options.
  unfold limits_typing in H6. remove_bools_options.
(* Qed. *) Admitted.

Lemma inst_typing_elem_ttype: forall s i x C a etype elem,
  inst_typing s i C ->
  List.nth_error i.(inst_elems) x = Some a ->
  List.nth_error C.(tc_elem) x = Some etype ->
  List.nth_error s.(s_elems) (N.to_nat a) = Some elem ->
  eleminst_type elem = etype.
Proof.
  move => s i x C a etype elem HIT HILU HCLU HSLU.
  destruct s, i, C, tc_local, tc_label, tc_return, tc_ref => //=;
  unfold inst_typing, typing.inst_typing in * => //=;
  remove_bools_options; simpl in * => //=.

  unfold elemi_typing in H1.
  eapply all2_projection in H1; eauto.
  remove_bools_options; eauto.
  unfold lookup_N in Hoption.
  rewrite Hoption in HSLU. inversion HSLU. subst.
  unfold elem_typing in H7. by move/eqP in H7.
Qed.

Lemma store_typing_elemv_type: forall s i C j a elem elemv,
  store_typing s ->
  inst_typing s i C ->
  List.nth_error s.(s_elems) (N.to_nat a) = Some elem ->
  List.nth_error (eleminst_elem elem) j = Some elemv ->
  eleminst_type elem = typeof_ref elemv.
Proof.
  move => s i C j a elem elemv HST HIT HSLU HTLU.
  destruct s, i, C, tc_local, tc_label, tc_return, tc_ref => //=;
  unfold inst_typing, typing.inst_typing in * => //=;
  remove_bools_options; simpl in * => //=.
  destruct HST as [_ [_ [_ [_ [HSTe _]]]]].
  move/List.Forall_forall in HSTe.
  destruct (HSTe elem);
    first (by eapply List.nth_error_In; eauto);
    first by destruct j => //=.
  
  induction j => //=; simpl in HTLU.
  - inversion HTLU. subst.
    unfold refcl_agree in H6.
    destruct (elemv) => //=.
    destruct H6 => //=.
  - move/List.Forall_forall in e.
    apply List.nth_error_In in HTLU.
    apply e in HTLU.
    unfold refcl_agree in HTLU.
    destruct (elemv) => //=.
    destruct HTLU => //=.
Qed.

Lemma store_typing_tabv_type: forall s i C j a tab tabv,
  store_typing s ->
  inst_typing s i C ->
  List.nth_error s.(s_tables) (N.to_nat a) = Some tab ->
  List.nth_error (tableinst_elem tab) j = Some tabv ->
  tt_elem_type (tableinst_type tab) = typeof_ref tabv.
Proof.
  move => s i C j a tab tabv HST HIT HSLU HTLU.
  destruct s, i, C, tc_local, tc_label, tc_return, tc_ref => //=;
  unfold inst_typing, typing.inst_typing in * => //=;
  remove_bools_options; simpl in * => //=.
  destruct HST as [_ [HSTt _]].
...
Lemma global_type_reference: forall s i j C v t,
  ...
  typeof v = t.
Proof.
  move => s i j C v t HST HIT Hvref Htref.
  ...
  move/andP in HIT. destruct HIT.
  remove_bools_options.
  destruct HST as [_ [_ [_ [HSTg _]]]].
  eapply all2_projection in H4; eauto.
  unfold globali_agree, option_map, global_typing in H4.
  rewrite Hoption0 in H4. move/eqP in H4.
  inversion H4. move/eqP in H7. subst. simpl in *.
  unfold global_agree in HSTg.
  rewrite -> List.Forall_forall in HSTg.
  apply List.nth_error_In in Hoption0.
  apply/eqP. apply (HSTg g1) => //=.
Qed.
...
Lemma memi_typing_extension: forall m0 m1 n m,
  size m0 <= size m1 ->
  memi_typing m0 n m ->
...
Proof.
  ...
  assert (size m0 = size (List.firstn (length (m0)) m1)) as HSize;
    first by eapply all2_size; eauto.
  unfold memi_typing.
  unfold memi_typing in H1. unfold mem_extension in H2.
  remove_bools_options.
  unfold option_map.
  ...
  rewrite H0. 
  eapply all2_projection in H2; [| by apply Hoption | by apply HN ].
  move/andP in H2. destruct H2. unfold limits_extension in H1.
  ...
  - unfold limits_typing in *. remove_bools_options. 
    apply/andP. split => //=; last by rewrite -H5; by apply/eqP.
    
    clear H2 H5.
    (* memi_typing m1 n m given it holds for m0 and m1 extension of m0 *)
    (* since m1 extension, it may have grown since m0;
        no guarantee it still respects m *)
      admit.
(* Qed. *) Admitted.

Lemma mem_extension_C: forall sm sm' im tcm,
  size sm <= size sm' ->
  all2 (memi_typing sm) im tcm ->
...
Proof.
  ...
  eapply memi_typing_extension; eauto.
Qed.

Lemma tabi_typing_extension: forall t0 t1 n t,
  size t0 <= size t1 ->
  tabi_typing t0 n t ->
...
Proof.
  ...
  assert (size t0 = size (List.firstn (length (t0)) t1)) as HSize;
    first by eapply all2_size; eauto.
  unfold tabi_typing.
  unfold tabi_typing in H1. unfold table_extension in H2.
  induction t, t0, t1 => //=;
  ...
  unfold limits_extension in H. remove_bools_options.
  ...
  - inversion HN; inversion Hoption; subst t3 t4.
    ...
    apply/andP. split => //; last by apply/eqP; rewrite -H11.
    admit.
    (* lim_min ...t1 <= lim_min tt_limits
    given: lim_min ...t <= lim_min tt_limits
      and: lim_min ...t <= lim_min ...t1  INSTEAD OF EQ *)
    (* same as argument above for memory *)
  - assert (List.nth_error t0 n0 <> None) as Ht0len;
    ...
    + rewrite H11 H5. apply/andP.
      assert (lim_max tt_limits == lim_max tt_limits); first by apply/eqP.
      rewrite H13. clear H13. simpl.
      split => //; first by apply Nat.leb_le in H0; lias.
      apply/andP. split => //; last by apply/eqP; rewrite -H10.
      admit.
      (* lim_min ...t1 <= lim_min tt_limits
      given: lim_min ...t <= lim_min tt_limits
        and: lim_min ...t <= lim_min ...t1  INSTEAD OF EQ *)
      (* same as argument above for memory *)
    + simpl.
      assert (tt_elem_type == tt_elem_type); first by apply/eqP.
      rewrite H13. clear H13. apply/eqP. by rewrite -H4.
(* Qed. *) Admitted.

Lemma tab_extension_C: forall st st' it tct,
  size st <= size st' ->
  all2 (tabi_typing st) it tct ->
...
Proof.
  ...
  by eapply tabi_typing_extension; eauto.
Qed.

Lemma elem_agree_extension: forall es0 es1 n er,
  size es0 <= size es1 ->
  elemi_typing es0 n er ->
  all2 elem_extension es0 (List.firstn (length (es0)) es1) ->
  elemi_typing es1 n er.
Proof.
  move => es0 es1 n er HLen H1 H2.
  assert (size es0 = size (List.firstn (length (es0)) es1)) as HSize;
    first by eapply all2_size; eauto.
  unfold elemi_typing, option_map, elem_typing.
  unfold elemi_typing, option_map, elem_typing in H1.
  unfold elem_extension in H2.
  remove_bools_options.

  assert (lookup_N es0 n <> None);
    first by rewrite Hoption.
  apply lookup_N_Some in H.
  repeat rewrite -length_is_size in HSize.

  assert (lookup_N es1 n <> None);
    first by apply lookup_N_Some;
    repeat rewrite -length_is_size in HLen; lias.
  destruct (lookup_N es1 n) eqn:E1 => //=.
...
Proof.
  ...
  - simpl in HA. remove_bools_options.
    ...
    unfold elemi_typing in H. remove_bools_options.
    ...
    + by eapply elem_agree_extension; eauto.
      (* unfold elemi_typing, option_map, lookup_N, elem_typing.
      assert (List.nth_error se' (N.to_nat a) = Some e0);
        first by eapply nth_error_firstn; eauto.
      unfold elemi_typing, option_map, lookup_N, elem_typing in H2.
...
Proof.
  ...
  assert (size d0 = size (List.firstn (length (d0)) d1)) as HSize;
    first by eapply all2_size; eauto.
  unfold datai_typing, option_map.
  unfold datai_typing, option_map in H1.
...
Proof.
  ...
  - simpl in HA. remove_bools_options.
    edestruct IHie; eauto.
    remember H as H'. clear HeqH'.
    unfold datai_typing in H. remove_bools_options.
...
Proof.
  ...
  destruct x. unfold tab_size, limits_extension.
  apply/andP. split => //=.
Qed.
...
Proof.
  ...
  remove_bools_options. by move/eqP in H7.
Qed.
  ...
  - assert ((n.+1 < length (t :: stabs))%coq_nat); first by rewrite -List.nth_error_Some; rewrite HN.
    ...
    + unfold table_extension, limits_extension.
      apply/andP; split => //. apply/andP; split => //;
        last by (apply/andP; split => //; apply/andP; split => //).
...
Proof.
  ...
  - rewrite H2 => //=. unfold limits_extension.
    apply/andP. split => //.
Qed.
...
Proof.
  ...
  - move : HMGrow.
    ...
    + unfold limits_extension. simpl. apply/andP.
  ...
  - inversion HMGrow; subst; clear HMGrow.
    ...
    + unfold limits_extension. simpl. apply/andP.
      ...
      (* Same proof structure in both cases *)
      (* lim_min (meminst_type m) <= (mem_size m + c)%N
         to be proven but no information on lim_min *)
(* Qed. *) Admitted.
...
Proof.
  ...
  - apply/eqP. f_equal. inversion HGrow.
    ...
    + simpl. apply/andP. split => //. unfold limit_valid in HLimMax.
    (* tab_extension definition comparing lim min here *)
    (* same argument as with memory above *)
    admit.
Admitted.
...
Lemma store_invariant_extension_store_typed: forall s s',
  ...
  store_typing s'.
Proof.
  move => s s' HST Hext HF HT HM HG HE.
  ...
  - rewrite -> List.Forall_forall. move => x HIN.
    ...
    destruct HIN. unfold cl_type_check_single.
...
Lemma store_table_extension_store_typed: forall s s',
  ...
  store_typing s'.
Proof.
  move => s s' HST Hext HF HM HT HG HE.
  remember HST as HST'. clear HeqHST'.
  remember Hext as Hext'. clear HeqHext'.
  unfold store_typing in HST'.
  unfold store_extension, operations.store_extension in Hext'.
...
Lemma store_global_extension_store_typed: forall s s',
  ...
  (s_mems s = s_mems s') ->
  (s_tables s = s_tables s') ->
  List.Forall global_agree (s_globals s') ->
  (s_elems s = s_elems s') ->
  store_typing s'.
Proof.
  move => s s' HST Hext HF HM HT HG HE.
...
Lemma store_elem_extension_store_typed: forall s s',
  store_typing s ->
  store_extension s s' ->
  (s_funcs s = s_funcs s') ->
  (s_mems s = s_mems s') ->
  (s_tables s = s_tables s') ->
  (s_globals s = s_globals s') ->
  List.Forall (elem_agree s) (s_elems s') ->
  store_typing s'.
Proof.
  move => s s' HST Hext HF HM HT HG HE.
  remember HST as HST'. clear HeqHST'.
...
(* add bounds check to tab_update
    use set_nth_table_preserve_type instead (prove by induction on index) *)
Lemma set_tab_agree:
...
Proof.
  ...
  - (* update glob *)
    ...
    remember {|
      s_funcs := s_funcs s;
      s_tables := s_tables s;
      datatypes.s_mems := datatypes.s_mems s;
      datatypes.s_globals :=
        set_nth {| g_type := g_type g1; g_val := v |}
          (datatypes.s_globals s) (N.to_nat g0)
          {| g_type := g_type g1; g_val := v |};
      s_elems := s_elems s;
      s_datas := s_datas s
    |} as s'.
    
    assert (store_extension s s') as Hext;
      last (split => //=; eapply store_global_extension_store_typed;
      eauto; destruct s, s' => //=; try by inversion Heqs').
  ...
  - (* elem_drop *) 
    ...
    remember {|
      ...
      s_tables := s_tables s;
      datatypes.s_mems := datatypes.s_mems s;
      datatypes.s_globals := datatypes.s_globals s;
      s_elems :=
        set_nth
          {|
            eleminst_type := eleminst_type e0; eleminst_elem := [::]
          |} (s_elems s) (N.to_nat e)
          {|
            eleminst_type := eleminst_type e0; eleminst_elem := [::]
          |};
      s_datas := s_datas s
    ...
    assert (store_extension s s') as Hext;
      last (split => //=; eapply store_elem_extension_store_typed;
      eauto; destruct s, s' => //=; try by inversion Heqs').
  ...
  - (* update memory : store none *) 
    ...
    unfold store_typing in HST.
    destruct s => //=.
    destruct HST as [_ [_ H11]]. destruct H11 as [H11 _].
    simpl in H1.
...
Proof.
  ...
  - invert_e_typing. convert_et_to_bet. invert_be_typing. simpl in *.
    apply inst_t_context_local_empty in HIT1. rewrite HIT1 in H2.
    rewrite H1. rewrite set_nth_map => //.
    rewrite set_nth_same_unchanged => //=.
  - eapply lfilled_es_type_exists in HType; eauto.
    destruct HType as [lab' [t1s' [t2s' Het]]]. 
...
Proof.
  ...
  - (* ref_func *)
    convert_et_to_bet.
    apply Ref_func_typing in HType.
  ...
  - (* block *)
    ...
    split_et_composition. invert_e_typing. convert_et_to_bet.
  ...
  - (* Invoke host *)
    ...
    unfold inst_typing, typing.inst_typing in HIT2.
  ...
  - (* Table Init Step *)
    ...
    assert (eleminst_type elem = tt_elem_type ttype);
      first by eapply inst_typing_elem_ttype; eauto.
    rewrite -H8.
    eapply store_typing_elemv_type; eauto.
...
Proof.
  ...
  assert (store_extension s s' /\ store_typing s');
    first by apply upd_label_unchanged_typing in H7;
      eapply store_extension_reduce; eauto.
  destruct H1.
  ...
  eapply mk_s_typing; eauto;
    first by eapply mk_frame_typing; eauto;
      apply reduce_inst_unchanged in HReduce.
  fold_upd_context.
